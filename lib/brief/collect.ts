// ═════════════════════════════════════════════════════════════════════════
// The Thursday Brief — the collector (v0.6.28)
//
// Builds the complete deterministic fact block for one edition.
//
// TWO PROPERTIES THIS FILE EXISTS TO GUARANTEE:
//
//   1. ONE SECTION FAILING NEVER KILLS THE BRIEF. Sections run in parallel
//      and are settled independently. A module that is down produces a
//      section carrying a warning; every other section still reports. A
//      report that does not arrive teaches the team to stop expecting it.
//
...
//      reach past it to the database. Every number the eventual commentary
//      can cite is a number already printed in a table beneath it.
// ═════════════════════════════════════════════════════════════════════════
...
/** Sections in reading order. Adding a module to the Brief means adding one
 *  line here and one file under sections/ — nothing else in the pipeline
 *  changes. */
type SectionFn = (closedSince: Date, closedUntil: Date, aheadSince: Date, aheadUntil: Date) => Promise<BriefSection>

const SECTIONS: Array<{ key: string; title: string; run: SectionFn }> = [
  {
    key: 'candidates',
    title: 'Candidates',
    run: function (cs, cu) { return collectCandidates(cs, cu) },
  },
  {
    key: 'carematch360',
    title: 'CareMatch360',
    run: function (cs, cu) { return collectCarematch(cs, cu) },
  },
]
