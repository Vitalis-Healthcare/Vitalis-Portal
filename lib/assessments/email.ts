...
export interface AssignmentEmailParams {
  ...
  planType?:       'clinical' | 'ep_annual'
}

export async function sendAssignmentEmail(p: AssignmentEmailParams): Promise<void> {
  const firstName  = p.nurseName.split(' ')[0] || 'Nurse'
  const isEP       = p.planType === 'ep_annual'
  const planLabel  = isEP ? 'EP Annual Plan' : 'Assessment'
  const verb       = p.isReassignment ? 'reassigned to' : 'assigned to'
  const subject    = p.isReassignment
    ? `${planLabel} reassignment \u2014 ${p.clientName}`
    : `New ${isEP ? 'EP annual plan' : 'assessment'} assignment \u2014 ${p.clientName}`

  const rows: Array<[string, string]> = [
    ['Plan Type',            isEP ? 'Emergency Preparedness (Annual)' : 'Clinical Assessment'],
    ['Client',               p.clientName],
    ['Phone',                p.clientPhone || '\u2014'],
  ...
  const content = `
    ...
    <p style="color:#4A6070;font-size:14px;line-height:1.6;margin:0 0 22px;">
      You have been <strong>${verb}</strong> a client for ${isEP ? 'Emergency Preparedness' : 'assessment'} scheduling. Details below.
    </p>
...
export async function sendWeeklyDigestEmail(p: WeeklyDigestParams): Promise<void> {
  ...
  const tableRows = p.items.map(item => `
    <tr>
      <td style="padding:10px 12px;font-size:13px;color:#1A2E44;border-bottom:1px solid #F1F5F9;font-weight:600;white-space:nowrap;">
        ${fmtDate(item.scheduledDate)}
        ${item.planType === 'ep_annual' ? '<div style="font-size:10px;color:#0E7C7B;font-weight:700;margin-top:2px;">EP ANNUAL</div>' : ''}
      </td>
  ...
  const bodyTable = p.items.length === 0
    ? `<div style="background:#F0FDF4;border:1px solid #BBF7D0;border-radius:8px;padding:16px 20px;margin-bottom:24px;text-align:center;font-size:14px;color:#15803D;">&#x2705; No assessments scheduled this week.</div>`
  ...
  const content = `
    <h2 style="font-size:18px;color:#1A2E44;margin:0 0 6px;">Hi ${firstName},</h2>
    <p style="color:#4A6070;font-size:14px;line-height:1.6;margin:0 0 22px;">Here are your assessments for the week of <strong>${p.weekLabel}</strong>.</p>
    ${bodyTable}
    <div style="text-align:center;margin-bottom:24px;">
      <a href="${PORTAL_URL}/assessments/calendar" style="display:inline-block;padding:12px 32px;background:#0E7C7B;color:#fff;text-decoration:none;border-radius:8px;font-weight:700;font-size:14px;">Open Calendar &rarr;</a>
    </div>`
...
export async function sendMonthlyScheduleEmail(p: MonthlyScheduleParams): Promise<void> {
  ...
  const bodyTable = p.items.length === 0
    ? `<div style="background:#F0FDF4;border:1px solid #BBF7D0;border-radius:8px;padding:16px 20px;margin-bottom:24px;text-align:center;font-size:14px;color:#15803D;">&#x2705; No assessments scheduled for ${p.monthLabel}.</div>`
    : `<div style="overflow-x:auto;margin-bottom:24px;">
         <table style="width:100%;border-collapse:collapse;border:1px solid #E2E8F0;border-radius:8px;overflow:hidden;">
           <thead><tr style="background:#F8FAFC;">
             <th style="padding:10px 12px;font-size:11px;color:#94A3B8;text-align:left;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;">Date</th>
  ...
  const content = `
    <h2 style="font-size:18px;color:#1A2E44;margin:0 0 6px;">Hi ${firstName},</h2>
    <p style="color:#4A6070;font-size:14px;line-height:1.6;margin:0 0 22px;">Here is your full assessment schedule for <strong>${p.monthLabel}</strong>.</p>
    ${bodyTable}
    <div style="text-align:center;margin-bottom:24px;">
      <a href="${PORTAL_URL}/assessments/calendar" style="display:inline-block;padding:12px 32px;background:#0E7C7B;color:#fff;text-decoration:none;border-radius:8px;font-weight:700;font-size:14px;">Open Calendar &rarr;</a>
    </div>`
