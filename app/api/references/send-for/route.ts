// app/api/references/send-for/route.ts
// Admin/supervisor/staff send a reference request on behalf of a caregiver.
// Called from the caregiver profile page in the Staff Portal.
...
export async function POST(request: Request) {
  ...
  const { caregiverId, slot, reference_type, referee_name, referee_email, referee_phone, referee_org } = await request.json()

  if (!caregiverId || !slot || !referee_email || !reference_type) {
    return NextResponse.json({ error: 'caregiverId, slot, referee_email and reference_type required' }, { status: 400 })
  }
  ...
  // Get caregiver name
  const { data: caregiver } = await svc.from('profiles').select('full_name').eq('id', caregiverId).single()
  const caregiverName = caregiver?.full_name || 'A Vitalis Healthcare caregiver'
...
<div style="max-width:600px;margin:0 auto;padding:32px 16px;">
  ...
  <div style="background:#fff;padding:32px;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 12px 12px;">
    ...
    <p style="color:#4A6070;font-size:14px;line-height:1.7;margin:0 0 16px;">
      <strong>${caregiverName}</strong> has applied to Vitalis Healthcare Services, LLC and has submitted your name as a reference. We would appreciate your cooperation in completing our <strong>${formType}</strong>. Your response will be kept in the strictest confidence.
    </p>
