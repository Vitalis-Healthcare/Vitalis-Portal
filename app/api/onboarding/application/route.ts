...
// submits it ('submit'). Field sanitization is shared with the staff edit route
// via buildApplicationRow(). On first save: test_passed -> applying; on submit:
...
export async function POST(req: NextRequest) {
  ...
  const fields = buildApplicationRow(application as Record<string, unknown>)

  if (action === 'submit') {
    const required = ['legal_first_name', 'legal_last_name', 'phone', 'email', 'signature_name']
  ...
  if (action === 'submit') { row.attested = true; row.signed_at = nowIso; row.submitted_at = nowIso }
