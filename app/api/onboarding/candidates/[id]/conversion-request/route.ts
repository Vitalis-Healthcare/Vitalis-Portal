...
async function viewerGate(): Promise<{ ok: true; viewer: Viewer } | { ok: false; res: NextResponse }> {
  try {
    ...
    return { ok: true, viewer: { userId: user.id, role } }
  } catch {
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: cand } = await svc
    .from('onb_candidates')
    .select('id, status, converted_to_profile_id')
    .eq('id', id)
    .maybeSingle()
  ...
  return NextResponse.json({ success: true, request_id: reqRow.id, status: 'awaiting_approval' })
}
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (decision === 'return') {
    ...
    return NextResponse.json({ success: true, decision: 'returned', status: 'in_review', note })
  }
