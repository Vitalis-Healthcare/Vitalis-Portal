...
// single-use magic-link token, and emails the invite via Resend. As of
// v0.6.35 the invite is track-aware: the full track sends the competency-test
// invite (today's flow, unchanged); the application-only track sends a link
// straight to the application. Resend always honours the stored track.
import { NextRequest, NextResponse } from 'next/server'
import crypto from 'crypto'
import { createClient } from '@/lib/supabase/server'
import { createServiceClient } from '@/lib/supabase/service'
import { normalizeTrack, type CandidateTrack } from '@/lib/onboarding/application'
...
// Tracks a coordinator may choose at invite time. 'documents_only' joins this
// list in v0.6.36 when its candidate-facing upload page ships — accepting it
// earlier would email a link to a page that does not exist yet.
const INVITABLE_TRACKS = ['full', 'application_only'] as const

function hashToken(raw: string) {
  return crypto.createHash('sha256').update(raw).digest('hex')
}

/** Per-track email copy. The link itself is built in sendInvite. */
function trackCopy(track: CandidateTrack) {
  if (track === 'application_only') {
    return {
...
function buildInviteEmail(opts: { firstName: string; link: string; track: CandidateTrack }) {
  const { firstName, link, track } = opts
...
async function sendInvite(
  to: string, firstName: string, rawToken: string, track: CandidateTrack,
): Promise<{ ok: boolean; error?: string }> {
  if (!RESEND_KEY) return { ok: false, error: 'Email service not configured.' }
  // Every track except application_only gets the test link. 'documents_only'
  // cannot be selected yet (INVITABLE_TRACKS), and an unknown value has been
  // normalised to 'full' before this is called, so the fallback is safe.
  const link = track === 'application_only'
    ? `${PORTAL_URL}/onboarding/application?token=${rawToken}`
    : `${PORTAL_URL}/onboarding/test?token=${rawToken}`
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [to],
        bcc: [TEAM_NOTIFY],
        subject: trackCopy(track).subject,
        html: buildInviteEmail({ firstName, link, track }),
      }),
    })
    if (!res.ok) {
...
export async function POST(req: NextRequest) {
  ...
  if (action === 'resend') {
    ...
    // Resend honours the track the candidate was invited on.
    const sent = await sendInvite(cand.email, cand.first_name, rawToken, normalizeTrack(cand.track))
  ...
  if (!(INVITABLE_TRACKS as readonly string[]).includes(requestedTrack)) {
  ...
  const sent = await sendInvite(email, first_name, rawToken, track)
