...
//
// Flow:
//   1. svc.auth.admin.createUser({ email_confirm: true })  — creates auth user, NO Supabase email
//   2. handle_new_user trigger                             — auto-creates profiles row
//   3. svc.auth.admin.generateLink({ type: 'recovery' })  — password-set link, valid 24h
//   4. Resend                                             — branded invite email with the link
//
// User clicks link → Supabase verifies token, establishes session → redirects to
// /update-password → user sets their password → redirects to /dashboard.
// The existing update-password page handles this perfectly with no changes needed.
...
<div style="max-width:560px;margin:0 auto;padding:32px 16px;">
  ...
  <div style="background:#fff;padding:32px;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 12px 12px;">
    ...
    <p style="color:#4A6070;font-size:14px;line-height:1.6;margin:0 0 24px;">
      Click the button below to set your password and sign in. The link is valid for <strong>24 hours</strong>.
    </p>

    <div style="text-align:center;margin-bottom:28px;">
      <a href="${setPasswordLink}" style="display:inline-block;padding:14px 40px;background:#0E7C7B;color:#fff;text-decoration:none;border-radius:8px;font-weight:700;font-size:15px;">
    ...
    <div style="background:#FEF3EA;border:1px solid #F4A26144;border-radius:8px;padding:12px 16px;margin-bottom:24px;">
      <div style="font-size:12px;color:#92400E;line-height:1.6;">
        ⚠️ <strong>This link expires in 24 hours.</strong> If it has expired, go to the portal login page and click <em>"Forgot password?"</em> to get a new link.
      </div>
    </div>
    ...
    <p style="color:#94A3B8;font-size:12px;margin:0;line-height:1.6;">Do not share this link. It is single-use and gives direct access to your account.</p>
  </div>
...
export async function POST(req: NextRequest) {
  ...
  if (!RESEND_KEY) {
    return NextResponse.json({ error: 'RESEND_API_KEY is not configured in Vercel environment variables.' }, { status: 500 })
  }
  ...
  // ── Step 1: Create auth user (silent — no Supabase email sent) ────────────
  ...
  // ── Step 2: Profile auto-created by handle_new_user trigger ──────────────
  // Belt-and-braces upsert in case trigger hasn't fired yet
  if (authData?.user?.id) {
  ...
  // ── Step 3: Generate password recovery link ───────────────────────────────
  // type: 'recovery' → user clicks → session established → redirect to
  // /update-password → user sets password → /dashboard
  // This reuses the existing update-password page with no changes.
  ...
  if (linkError || !linkData?.properties?.action_link) {
    ...
    return NextResponse.json({
      ...
      message: 'Account created but invite link generation failed. The user can use "Forgot password?" on the login page.',
    })
  }

  const setPasswordLink = linkData.properties.action_link

  // ── Step 4: Send branded invite email via Resend ──────────────────────────
  ...
  if (!resendRes.ok) {
    ...
    console.error('[invite] Resend error:', resendErr)
    return NextResponse.json({
      status: 'created_email_failed',
      message: 'Account created but the invite email failed. Check Resend logs.',
    })
  }
