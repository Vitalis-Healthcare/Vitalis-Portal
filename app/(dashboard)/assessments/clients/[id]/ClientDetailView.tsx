...
const PAYER_TYPES = ['Private Pay','Medicaid','Medicare','Medicaid Waiver','CareFirst','Wellpoint','United Healthcare','Aetna','BCHD','Other']
...
function StatusBadge({ status }: { status: string }) {
  const m: Record<string, [string, string, string]> = {
    scheduled: ['#EFF6FF','#1D4ED8','#BFDBFE'], completed: ['#F0FDF4','#15803D','#86EFAC'],
    overdue:   ['#FEF2F2','#B91C1C','#FECACA'], cancelled: ['#F9FAFB','#6B7280','#E5E7EB'],
    active:    ['#F0FDF4','#15803D','#86EFAC'], inactive:  ['#FFF7ED','#C2410C','#FED7AA'],
    discharged:['#F9FAFB','#6B7280','#E5E7EB'],
  }
  const [bg, color, border] = m[status] ?? m.scheduled
  return <span style={{ display:'inline-block', padding:'3px 12px', borderRadius:12, fontSize:12, fontWeight:600, background:bg, color, border:`1px solid ${border}`, textTransform:'capitalize' }}>{status}</span>
}
...
function PlanBadge({ planType }: { planType: 'clinical'|'ep_annual'|'unknown' }) {
  if (planType === 'ep_annual') return <span style={{ fontSize:10, fontWeight:700, color:'#0E7C7B', background:'#E6F4F4', padding:'2px 6px', borderRadius:6, border:'1px solid #B2E0DF', whiteSpace:'nowrap' }}>EP</span>
  return null
}
...
export default function ClientDetailView({
  ...
  const inp = { width:'100%', padding:'8px 12px', border:'1px solid #D1D9E0', borderRadius:7, fontSize:13, color:'#1A2E44', background:'#fff', boxSizing:'border-box' as const }
  const lbl = { display:'block', fontSize:12, fontWeight:600 as const, color:'#4A6070', marginBottom:4 }
  const eBtn = { padding:'3px 10px', background:'transparent', border:'1px solid #D1D9E0', color:'#0E7C7B', borderRadius:6, fontSize:11, fontWeight:600 as const, cursor:'pointer' }
  ...
  // Archive = set status to discharged (reversible via Edit Client)
  const archiveClient = async () => {
    ...
    try {
      ...
      if (!res.ok) { const d = await res.json(); setErr(d.error ?? 'Failed to archive.'); return }
      router.push('/assessments/clients')
    } catch { setErr('Unexpected error.') } finally { setBusy(false) }
  }

  // Delete = hard delete, cascades via ON DELETE CASCADE
  const deleteClient = async () => {
    if (deleteConfirmName !== client.full_name) return
    setBusy(true); setErr(null)
    try {
      const res = await fetch(`/api/assessments/clients/${client.id}`, { method:'DELETE' })
      if (!res.ok) { const d = await res.json(); setErr(d.error ?? 'Failed to delete.'); return }
      router.push('/assessments/clients')
    } catch { setErr('Unexpected error.') } finally { setBusy(false) }
  }
            ...
            {next && (
              <div style={{ marginTop:10, padding:'8px 12px', background: isEP ? '#F8FAFC' : '#EFF6FF', borderRadius:7, border:`1px solid ${isEP ? '#E2E8F0' : '#BFDBFE'}` }}>
                ...
                <div style={{ fontSize:11, color:'#4A6070', marginTop:2 }}>
                  {(() => { const d = daysUntil(next.scheduled_date); if (d < 0) return <span style={{ color:'#B91C1C', fontWeight:600 }}>{Math.abs(d)} days overdue</span>; if (d === 0) return <span style={{ color:'#D97706', fontWeight:600 }}>Due today</span>; return `in ${d} day${d !== 1 ? 's' : ''}` })()}
                </div>
              </div>
            )}
          </>
        ) : (
          <div style={{ color:'#D97706', fontSize:13, padding:'4px 0' }}>⚠ No {isEP ? 'EP plan' : 'clinical schedule'}. {canEdit ? 'Click "Assign" to set up.' : 'Contact your supervisor.'}</div>
        )}
      </div>
    )
  }
    ...
    <div style={{ padding:'32px 32px 64px', maxWidth:960, margin:'0 auto' }}>
      ...
      <div style={{ marginBottom:24 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginTop:8 }}>
          ...
          <div style={{ display:'flex', gap:10 }}>
            {canEdit && <button onClick={() => { setShowEdit(true); setErr(null) }} style={{ padding:'8px 16px', background:'#F8FAFC', border:'1px solid #D1D9E0', color:'#1A2E44', borderRadius:7, fontSize:13, fontWeight:600, cursor:'pointer' }}>✎ Edit Client</button>}
            {canEdit && pendingAssessments.length > 0 && <button onClick={() => { setShowEmergency(true); setErr(null) }} style={{ padding:'8px 16px', background:'#FEF2F2', border:'1px solid #FECACA', color:'#B91C1C', borderRadius:7, fontSize:13, fontWeight:600, cursor:'pointer' }}>+ Emergency Assessment</button>}
          </div>
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:24 }}>
        <div style={{ background:'#fff', border:'1px solid #E2E8F0', borderRadius:12, padding:'20px 22px' }}>
          ...
          {([['Date of Birth',fmt(client.date_of_birth)],['Phone',client.phone??'—'],['Address',[client.address,client.city,client.state,client.zip].filter(Boolean).join(', ')||'—'],['Payer',client.payer_type??'—'],['AxisCare ID',client.axiscare_id??'—']] as [string,string][]).map(([l,v]) => (
      ...
      {pendingAssessments.length > 0 && (
        <div style={{ background:'#fff', border:'1px solid #E2E8F0', borderRadius:12, overflow:'hidden', marginBottom:20 }}>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
            <thead><tr style={{ background:'#F8FAFC' }}>{['Due Date','Plan','Type','Nurse','Status',''].map(h => <th key={h} style={{ padding:'9px 16px', textAlign:'left', fontSize:11, fontWeight:700, color:'#4A6070', textTransform:'uppercase', letterSpacing:'0.5px', borderBottom:'1px solid #E2E8F0' }}>{h}</th>)}</tr></thead>
            <tbody>
              {pendingAssessments.map((a, idx) => (
                <tr key={a.id} style={{ borderBottom: idx < pendingAssessments.length - 1 ? '1px solid #F1F5F9' : 'none' }}>
                  <td style={{ padding:'12px 16px', fontWeight:600, color:'#1A2E44' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:6 }}>{fmt(a.scheduled_date)}{a.is_initial && <InitialBadge />}</div>
                    {daysUntil(a.scheduled_date) < 0 && <div style={{ fontSize:11, color:'#B91C1C', fontWeight:600, marginTop:2 }}>{Math.abs(daysUntil(a.scheduled_date))}d overdue</div>}
                  </td>
                  ...
                  <td style={{ padding:'12px 16px' }}><button onClick={() => { setShowComplete(a.id); setCompleteNotes(''); setErr(null) }} style={{ padding:'5px 12px', background:'#F0FDF4', border:'1px solid #86EFAC', color:'#15803D', borderRadius:6, fontSize:12, fontWeight:600, cursor:'pointer' }}>Mark Complete</button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {pastAssessments.length > 0 && (
        <div style={{ background:'#fff', border:'1px solid #E2E8F0', borderRadius:12, overflow:'hidden', marginBottom:24 }}>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
            <thead><tr style={{ background:'#F8FAFC' }}>{['Scheduled','Plan','Completed','Type','Nurse','Completed By','Status'].map(h => <th key={h} style={{ padding:'9px 16px', textAlign:'left', fontSize:11, fontWeight:700, color:'#4A6070', textTransform:'uppercase', letterSpacing:'0.5px', borderBottom:'1px solid #E2E8F0' }}>{h}</th>)}</tr></thead>
            <tbody>
      ...
      {isAdmin && (
        <div style={{ padding:'20px 24px', background:'#FFF7ED', border:'1px solid #FED7AA', borderRadius:12 }}>
          ...
          <div style={{ display:'flex', gap:10, marginBottom:10 }}>
            <button onClick={() => { setShowArchive(true); setErr(null) }} style={{ padding:'8px 16px', background:'#FEF3C7', border:'1px solid #FDE68A', color:'#92400E', borderRadius:7, fontSize:13, fontWeight:600, cursor:'pointer' }}>
              📦 Archive Client
            </button>
            <button onClick={() => { setShowDelete(true); setDeleteConfirmName(''); setErr(null) }} style={{ padding:'8px 16px', background:'#FEF2F2', border:'1px solid #FECACA', color:'#B91C1C', borderRadius:7, fontSize:13, fontWeight:600, cursor:'pointer' }}>
              🗑 Delete Client
            </button>
          </div>
          <div style={{ fontSize:11, color:'#B45309' }}>
            <strong>Archive</strong> moves client to Discharged status — all records preserved, reversible via Edit Client. &nbsp;
      ...
      {showEdit && (
        <div style={{ position:'fixed', inset:0, background:'rgba(26,46,68,0.45)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000, padding:20 }}>
          <div style={{ background:'#fff', borderRadius:14, padding:28, width:600, maxWidth:'95vw', maxHeight:'90vh', overflowY:'auto' }}>
            ...
            <div style={{ marginBottom:14 }}><label style={lbl}>Full Name <span style={{ color:'#B91C1C' }}>*</span></label><input style={inp} value={editName} onChange={e => setEditName(e.target.value)} /></div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:14 }}>
              <div><label style={lbl}>Payer Type</label><select style={inp} value={editPayer} onChange={e => setEditPayer(e.target.value)}><option value="">— Select —</option>{PAYER_TYPES.map(p => <option key={p} value={p}>{p}</option>)}</select></div>
              <div><label style={lbl}>Status</label><select style={inp} value={editStatus} onChange={e => setEditStatus(e.target.value)}><option value="active">Active</option><option value="inactive">Inactive</option><option value="discharged">Discharged</option></select></div>
            </div>
      ...
      {showEmergency && (
        <div style={{ position:'fixed', inset:0, background:'rgba(26,46,68,0.4)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }}>
          <div style={{ background:'#fff', borderRadius:14, padding:28, width:440, maxWidth:'90vw' }}>
            ...
            <div style={{ background:'#FEF2F2', border:'1px solid #FECACA', borderRadius:8, padding:'10px 14px', fontSize:12, color:'#B91C1C', marginBottom:18 }}>⚠ When completed, this resets the next due date to today + {clinicalSchedule?.cadence_days ?? '—'} days.</div>
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(26,46,68,0.4)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }}>
          <div style={{ background:'#fff', borderRadius:14, padding:28, width:460, maxWidth:'90vw' }}>
            ...
            {editingPlan === 'ep_annual' ? (
              <div style={{ marginBottom:14 }}><label style={lbl}>Cadence</label><div style={{ padding:'8px 12px', border:'1px solid #D1D9E0', borderRadius:7, fontSize:13, color:'#4A6070', background:'#F8FAFC' }}>Annual (365 days) — fixed for EP plans</div></div>
            ) : (
              <div style={{ marginBottom:14 }}><label style={lbl}>Cadence</label><select style={inp} value={newCadence} onChange={e => setNewCadence(Number(e.target.value))}>{CADENCE_OPTIONS.map(d => <option key={d} value={d}>{d}-day</option>)}</select></div>
            )}
            {isNewSchedule && (
              <div style={{ marginBottom:14 }}><label style={lbl}>First Assessment Due Date</label><input type="date" style={inp} value={newFirstDue} onChange={e => setNewFirstDue(e.target.value)} /></div>
            )}
            {!isNewSchedule && nextAssessmentId && (
              <div style={{ marginBottom:14 }}>
                ...
                <div style={{ fontSize:11, color:'#8FA0B0', marginTop:4 }}>Adjust if the nurse&apos;s schedule requires a different date.</div>
              </div>
            )}
            {editingPlan === 'clinical' && (isNewSchedule || nextAssessmentId) && (
              <div style={{ marginBottom:18, display:'flex', alignItems:'flex-start', gap:10, padding:'12px 14px', background:'#FFFBEB', border:'1px solid #FDE68A', borderRadius:8 }}>
                <input type="checkbox" id="is-initial-check" checked={isInitialCheck} onChange={e => setIsInitialCheck(e.target.checked)} style={{ width:16, height:16, marginTop:2, accentColor:'#92400E', cursor:'pointer', flexShrink:0 }} />
                <label htmlFor="is-initial-check" style={{ fontSize:13, color:'#92400E', fontWeight:600, cursor:'pointer', margin:0 }}>
                  ...
                  <div style={{ fontSize:11, color:'#B45309', fontWeight:400, marginTop:2 }}>
                    {isNewSchedule ? "Check for clients whose first assessment has not happened yet." : "Check or uncheck to correct the initial assessment flag on the next pending visit."}
                  </div>
                </label>
              </div>
            )}
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(26,46,68,0.4)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }}>
          <div style={{ background:'#fff', borderRadius:14, padding:28, width:440, maxWidth:'90vw' }}>
            ...
            <p style={{ fontSize:13, color:'#4A6070', margin:'0 0 18px', lineHeight:1.6 }}>
              This client will be moved to <strong>Discharged</strong> status and removed from the active scheduling view. All assessment history is fully preserved. You can reverse this at any time via Edit Client.
            </p>
            {err && <div style={{ color:'#B91C1C', fontSize:12, marginBottom:12 }}>{err}</div>}
            <div style={{ display:'flex', gap:10, justifyContent:'flex-end' }}>
      ...
      {showDelete && (
        <div style={{ position:'fixed', inset:0, background:'rgba(26,46,68,0.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }}>
          <div style={{ background:'#fff', borderRadius:14, padding:28, width:460, maxWidth:'90vw' }}>
            ...
            <p style={{ fontSize:13, color:'#4A6070', margin:'0 0 10px' }}>
            ...
            />
            {deleteConfirmName && deleteConfirmName !== client.full_name && (
              <div style={{ fontSize:11, color:'#B91C1C', marginBottom:14 }}>Name does not match — check capitalisation.</div>
            )}
